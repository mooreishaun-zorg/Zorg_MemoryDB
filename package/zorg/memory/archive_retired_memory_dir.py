...
def archive_schema_path() -> Path:
    candidates = [
        BASE / 'db' / 'memory_file_archive_schema.sql',
        BASE / 'zorg-memorydb' / 'db' / 'memory_file_archive_schema.sql',
        BASE / 'Zorg_MemoryDB' / 'package' / 'zorg' / 'db' / 'memory_file_archive_schema.sql',
    ]
    for candidate in candidates:
        if candidate.exists():
            return candidate
    raise FileNotFoundError('memory_file_archive_schema.sql not found in known workspace locations')
...
def main():
    ...
    with conn:
        with conn.cursor() as cur:
            cur.execute(archive_schema_path().read_text())
            for path in files:
